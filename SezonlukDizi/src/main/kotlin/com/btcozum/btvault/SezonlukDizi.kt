...
class SezonlukDizi : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        ...
        "${mainUrl}/diziler.asp?siralama_tipi=id&kat=1&s="    to "Yabanci Diziler",
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val actors      = actorsReq.select("div.doubling div.ui").map { Actor(it.selectFirst("div.header")!!.text().trim(), fixUrlNull(it.selectFirst("img")?.attr("src"))) }
        ...
        for (sezon in episodesReq.select("table.unstackable")) {
            for (bolum in sezon.select("tbody tr")) {
                ...
                val epEpisode = bolum.selectFirst("td:nth-of-type(3)")?.text()?.substringBefore(".Bolum")?.trim()?.toIntOrNull()
