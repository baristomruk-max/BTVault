...
class KultFilmler : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = document.select("div.col-md-12 div.movie-box").mapNotNull { it.toSearchResult() }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val actors      = document.select("div.actors a").map { Actor(it.text()) }
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        for (iframe in iframes) {
            if (iframe.contains("vidmoly")) {
                val headers = mapOf("User-Agent" to "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36", "Sec-Fetch-Dest" to "iframe")
