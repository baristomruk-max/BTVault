...
class DiziPal : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        "${mainUrl}/diziler/son-bolumler"                          to "Son Bolumler",
    ...
    private fun Element.sonBolumler(): SearchResponse? {
        ...
        val episode   = this.selectFirst("div.episode")?.text()?.trim()?.replace(". Sezon ", "x")?.replace(". Bolum", "") ?: return null
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val year        = document.selectXpath("//div[text()='Yapim Yili']//following-sibling::div").text().trim().toIntOrNull()
        val description = document.selectFirst("div.summary p")?.text()?.trim()
        val tags        = document.selectXpath("//div[text()='Turler']//following-sibling::div").text().trim().split(" ").map { it.trim() }
        val duration    = Regex("(\\d+)").find(document.selectXpath("//div[text()='Ortalama Suresi']//following-sibling::div").text())?.value?.toIntOrNull()
